 ...
 * ======================================================================================================
   ...
   1.4.006	2021.12.21	최명호 		로그인시 valid 가 active 인 것만 처리함.
**********************************************************************************************************/
...
//        		DESC >> 최명호 : 아이디없이 Email 이 수신된 경우.
        	...
        	if(Misc.isEqualStringWithoutCase(clsAuthMemberSet.getValid(),CodeUtil.get("active").getStrCode()) == false) {
        		log.error("아이디[{}]가 사용가능한 상태가 아닙니다..",strId);
        		throw new Exception("error");
        	}
...
//        	DESC >> 사용자정보를 조회하고 인증을 진행한다.
        	...
        	log.info("Login : isAuthenticated [{}]",clsAuthentication.isAuthenticated(),clsAuthentication);
